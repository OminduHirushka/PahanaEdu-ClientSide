...
import { getAllOrders, updateOrderStatus, updatePaymentStatus } from "../../../state/order/orderAction";
import {
  ORDER_STATUS,
  ORDER_STATUS_LABELS,
  ORDER_STATUS_COLORS,
...
const ManagerOrders = () => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      // Filter only online orders made by customers
      const onlineOrders = ordersData.filter(order => 
  ...
  const confirmStatusUpdate = async () => {
    ...
    try {
      ...
      setNewStatus("");
      // Don't show additional success message since the action already shows one
    } catch (error) {
      console.error("Failed to update order status:", error);
      // Don't show additional error message since the action already shows one
    } finally {
  ...
  const handleGenerateInvoice = async (order) => {
    try {
      const customerData = users.find(user => user.accountNumber === order.customerAccountNumber) || {
